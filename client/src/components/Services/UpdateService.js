...
class UpdateService extends React.Component {
  ...
  handleSubmit = (e) => {
    ...
    const { name, price, length, description, id } = this.state;
    dispatch(updateService({ name, price, length, description }, id));
    this.setState({ open: false });
  }
...
const SimpleModalWrapped = withStyles(styles)(UpdateService);
